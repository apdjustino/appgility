...
const RegistrationConfig = () => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    runPrices: Yup.array().of(
      Yup.object().shape({
        price: Yup.number().min(0, "Minimum price is 0").required("Required")
      })
    ).min(1, "Must have at least one price").max(6, "No more than 6 prices allowed")
  });
            ...
            <div className="col-xl-6 col-md-12">
              ...
              <FieldArray name="runPrices">
                {({ push, remove }) => (
                  <div>
                    <div className="pb-3">
                      <button className="btn btn-white" type="button" onClick={() => push({price: ""})}>Add Price</button>
                    </div>
                    {typeof formik.errors.runPrices === "string" ? (
                      <Alert variant="danger">{formik.errors.runPrices}</Alert>
                    ) : null}
                      ...
                      <div className="d-flex pb-3">
                        <div className="me-3">
                          <Form.Control 
                            ...
                            isInvalid={!!formik.getFieldMeta(`runPrices[${index}].price`).error && !!formik.getFieldMeta(`runPrices[${index}].price`).touched}
                            
                          />
                          <Form.Control.Feedback type="invalid">{formik.getFieldMeta(`runPrices[${index}].price`).error}</Form.Control.Feedback>
                        </div>
