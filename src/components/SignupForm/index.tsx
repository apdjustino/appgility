...
const SignupForm = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        password: Yup.string()
            ...
            .matches(
                /(?=^.{8,}$)((?=.*\w)(?=.*[A-Z])(?=.*[0-9]))^.*/,
                "Password must be at least 8 characters and include a lowercase character, an uppercase character, and a number",
            ),
    });
        ...
        <>
            ...
            <Form noValidate onSubmit={formik.handleSubmit}>
                ...
                <Button size="lg" className="w-100 mb-3" type="submit">
                    {result.loading ? <Spinner animation="border" /> : "Sign up"}
                </Button>
                <p className="text-center">
                    <span className="text-muted text-center fs-3">
                        Already have an account?{" "}
                        <button type="button" className="btn btn-link p-0" onClick={() => loginWithRedirect()}>
                            Log in
                        </button>
                        .
                    </span>
