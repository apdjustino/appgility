                        ...
                        <div className="row align-items-end">
                            ...
                            <div className="col-auto">
                                <ul className="nav nav-tabs header-tabs fs-3">
                                    {!!data.getEventTrials
                                        ? data.getEventTrials.map(({ trialDate, trialId }) => {
                                              const label = parseTimeStamp(trialDate, "MMMM  do, y");
                                              return (
                                                  <li className="nav-item">
                                                      <Link
                                                          to={`/secretary/events/${params.eventId}/registration/${trialId}`}
                                                          className={`nav-link text-center ${pathname.includes(trialId) ? "active" : null}`}
                                                      >
                                                          {label}
                                                      </Link>
                                                  </li>
