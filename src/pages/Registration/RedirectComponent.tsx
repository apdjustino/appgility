...
const RedirectComponent = ({ redirectTrialId }: OwnProps) => {
  ...
  return !!redirectTrialId ? (
    ...
    <div className="d-flex flex-column justify-content-center align-items-center my-6">
      <h2 className="text-body fst-italic">No Trials Found. <Link className="link-primary" to={`/secretary/events/${eventId}/configuration/trials`}>Click here</Link> to add a new trial.</h2>          
    </div>
  )
}
